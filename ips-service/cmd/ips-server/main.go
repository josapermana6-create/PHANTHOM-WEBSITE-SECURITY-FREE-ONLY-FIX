...
func main() {
	...
	logger.Info("Starting Phantom IPS Service...")
	...
	if cfg.Server.EnableREST {
		...
		restServer := rest.NewServer(cfg, ipDetector, dbStore, redisStore, logger)
	...
	logger.Info("Phantom IPS Service is running. Press Ctrl+C to stop.")
	...
	// Graceful shutdown
	cancel()
	logger.Info("Phantom IPS Service stopped")
}
